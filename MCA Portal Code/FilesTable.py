...
def change_of_name_table(db_config, cin):
    try:
        connection = mysql.connector.connect(**db_config)
        cursor = connection.cursor()
        files_query = 'select * from documents where cin = %s and LOWER(document) like "%%change of name%%" and form_data_extraction_needed = "Y" and Category = "Certificates"'
        value = (cin,)
        cursor.execute(files_query, value)
        file_results = cursor.fetchall()
        cursor.close()
        ...
        if len(file_results) != 0:
            ...
            soup = 'No name history files available'
            return soup
    except Exception as e:
        print(f"Exception in generating Directors Table {e}")
        return None
